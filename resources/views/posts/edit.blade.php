...
<head>
    ...
    <title>Edit Berita — {{ $post->title }} — Portal Berita</title>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .breadcrumb-current { color: var(--ink-800); font-weight: 500; max-width: 200px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
        ...
        .post-status-badge.draft { background: var(--warn-soft); color: var(--warn); border: 1px solid #fcd34d; }
        .post-status-badge.published { background: var(--success-soft); color: var(--success); border: 1px solid #6ee7b7; }
        .post-status-badge-dot { width: 5px; height: 5px; border-radius: 50%; background: currentColor; }
        ...
        .btn-outline:hover { background: var(--surface-hover); border-color: var(--ink-200); transform: translateY(-1px); }

        .btn-ghost { background: transparent; color: var(--ink-400); border: 1px solid transparent; }
        .btn-ghost:hover { background: var(--surface-hover); color: var(--ink-600); }

        .btn-draft { background: var(--warn-soft); color: var(--warn); border: 1px solid #fcd34d; }
        ...
        .btn-live:hover { background: #d1fae5; transform: translateY(-1px); }
        ...
        .page-meta {
            display: flex; align-items: center; gap: 10px;
            font-size: 0.75rem; color: var(--ink-200); flex-wrap: wrap;
        }

        .page-meta-sep { color: var(--ink-100); }
        ...
        .panel-hd-title { font-size: 0.79rem; font-weight: 700; color: var(--ink-800); }
        ...
        .label { font-size: 0.77rem; font-weight: 600; color: var(--ink-600); letter-spacing: 0.01em; }
        ...
        .slug-prefix { font-size: 0.82rem; color: var(--ink-200); white-space: nowrap; flex-shrink: 0; }
        ...
        .editor-area[contenteditable=true]:empty:before {
            ...
            color: var(--ink-100); pointer-events: none; font-style: italic;
        }
        ...
        .seg-opt input[type="radio"]:checked + .seg-face.draft { background: var(--warn-soft); color: var(--warn); }
        .seg-opt input[type="radio"]:checked + .seg-face.publish { background: var(--success-soft); color: var(--success); }
        ...
        .tag-chip-remove {
            ...
            color: var(--indigo); opacity: 0.6; padding: 0;
        ...
        .tag-chip-input {
            ...
            font-family: 'DM Sans', sans-serif;
            font-size: 0.82rem; color: var(--ink-950);
            min-width: 90px; flex: 1; padding: 2px 0;
        ...
        .flash-success {
            ...
            background: var(--success-soft); border: 1px solid #6ee7b7;
            border-left: 4px solid var(--success);
            border-radius: var(--r-lg); font-size: 0.80rem; color: var(--success);
        ...
        .flash-close {
            margin-left: auto; color: var(--success); opacity: 0.5;
        ...
        @media (max-width: 1100px) { :root { --col-side-w: 270px; } }
    ...
    <header class="topbar">
        <div class="topbar-left">
            ...
            <nav class="breadcrumb">
                ...
                <span class="breadcrumb-current">Edit: {{ $post->title }}</span>
            </nav>
        </div>
        <div class="topbar-right">
            ...
            @if($post->status === 'published')
                <a href="{{ route('posts.show_public', $post->slug) }}" target="_blank" class="btn btn-live btn-sm" style="display:inline-flex; align-items:center; gap:6px; padding:6px 13px; font-size:0.78rem; font-weight:600; border-radius:var(--r-sm);">
    ...
    <main class="page-body">
        ...
        <div class="page-header">
            <div class="page-header-left">
                ...
                <div class="page-meta">
                    <span class="post-status-badge {{ $post->status === 'published' ? 'published' : 'draft' }}">
                        <span class="post-status-badge-dot"></span>
                        {{ $post->status === 'published' ? 'Terpublikasi' : 'Draft' }}
                    </span>
                    <span class="page-meta-sep">·</span>
                    <span>Dibuat {{ $post->created_at->translatedFormat('d F Y') }}</span>
                ...
                <div class="col-main">
                    ...
                    <div class="panel" style="overflow:visible;">
                        ...
                        <div style="padding: 0 18px 12px; display:flex; justify-content:space-between; align-items:center;">
                            <span class="field-hint">Slug URL artikel</span>
                    ...
                    <div class="panel">
                        ...
                        <div id="toolbarVisual" class="editor-toolbar">
                            <button type="button" class="tb-btn" onclick="fmt('bold')"><b>B</b></button>
                            <button type="button" class="tb-btn" onclick="fmt('italic')"><i>I</i></button>
                            <button type="button" class="tb-btn" onclick="fmt('underline')"><u>U</u></button>
                            <div class="tb-sep"></div>
                            <button type="button" class="tb-btn" onclick="fmt('formatBlock','h2')" style="font-size:0.70rem; font-weight:800;">H2</button>
                            <button type="button" class="tb-btn" onclick="fmt('formatBlock','h3')" style="font-size:0.70rem; font-weight:800;">H3</button>
                            <div class="tb-sep"></div>
                            <button type="button" class="tb-btn" onclick="fmt('insertUnorderedList')">
                            ...
                            <button type="button" class="tb-btn" onclick="fmt('insertOrderedList')">
                            ...
                            <div class="tb-sep"></div>
                            <button type="button" class="tb-btn" onclick="insertLink()">
                            ...
                            <button type="button" class="tb-btn" onclick="fmt('formatBlock','blockquote')">
                        ...
                        >{{ old('body_markdown', $post->body) }}</textarea>

                        <textarea name="body" id="bodyHidden" style="display:none;">{{ old('body', $post->body) }}</textarea>
                ...
                <div class="col-side">
                    ...
                    <div class="action-panel">
                        <button type="submit" name="status_submit" value="published" class="btn btn-primary" onclick="setStatus('published')">
                            ...
                            {{ $post->status === 'published' ? 'Simpan Perubahan' : 'Publish Sekarang' }}
                        </button>
                        ...
                        <input type="hidden" name="status" id="statusInput" value="{{ old('status', $post->status) }}" />
                        <div class="action-divider"></div>
                        <form method="POST" action="{{ route('posts.destroy', $post->id) }}" onsubmit="return confirm('Yakin ingin menghapus artikel ini? Tindakan ini tidak dapat dibatalkan.')" style="width:100%;">
                            @csrf
                            @method('DELETE')
                    ...
                    <div class="panel">
                        ...
                        <div class="panel-bd">
                            <div class="status-segment">
                                <label class="seg-opt">
                                    <input type="radio" name="status_display" value="draft" {{ old('status', $post->status) === 'draft' ? 'checked' : '' }} onchange="syncStatusDisplay(this.value)" />
                                ...
                                <label class="seg-opt">
                                    <input type="radio" name="status_display" value="published" {{ old('status', $post->status) === 'published' ? 'checked' : '' }} onchange="syncStatusDisplay(this.value)" />
                            ...
                            <p class="field-hint" style="margin-top:8px;" id="statusHint">
                                {{ $post->status === 'published' ? 'Post sedang tampil di halaman publik.' : 'Post disimpan sebagai draft — tidak tampil di publik.' }}
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="panel">
                        ...
                        <div class="panel-bd">
                            ...
                            <input type="hidden" name="tags" id="tagsHidden" value="{{ old('tags', $post->tags ?? '') }}" />
...
<script>
    let slugManuallyEdited = '{{ $post->slug }}' !== '' ? true : false;
    let isDirty = false;
    const tagList = [];
    ...
    function syncMarkdownToHidden() {
        document.getElementById('bodyHidden').value = document.getElementById('editorMarkdown').value;
        markDirty();
    }
    ...
    (function initExistingTags() {
        const existing = document.getElementById('tagsHidden').value;
        if (existing) {
    ...
    (function initWordCount() {
        const content = document.getElementById('editorVisual').innerText || '';
    ...
    (function initCharCounter() {
        ...
        updateCharCount(title, 'titleCounter', 120);
    })();
